"use client";
import NewsHighlights from "@/app/components/news-details/NewsHighlights";
import { useParams } from "next/navigation"; // Import useParams

const NewsDetailsContainer: React.FC<any> = () => {
  const params : {detail:string, locale:string}= useParams();
  const newsCards = [
    {
      id: 1,
      title:
  ...
  const compData = newsCards?.find(
    (v: any) => v.id === parseInt(params.detail),
  );


  
  const ARcompData = ARnewsCards?.find(
    (v: any) => v.id === parseInt(params.detail),
  );
  ...
  return (
    <div>
      <NewsHighlights
        data={
          params.locale === "en" ? compData : ARcompData
        }
        locale={params.locale}
      />
    </div>
