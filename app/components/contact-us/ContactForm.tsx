  ...
  return (
    <ScreenContainer>
      <div className="flex flex-col justify-between gap-[60px] py-[40px] sm:gap-[85px] sm:py-[50px] lg:flex-row lg:gap-[182px] lg:py-[100px]">
        <div className="flex  w-full max-w-full flex-col gap-[30px] sm:gap-[48px] lg:max-w-[323px] lg:gap-[50px] ">
          ...
          <div className="flex flex-col gap-[16px] ">
            <h5 className={`${setFontFamily(data?.locale || "en", "medium")} text-[18px] font-medium	leading-[23px] text-[#9E8557] sm:text-[24px] sm:leading-[31px]`}>
              {data.emailHeading}
            </h5>
            <hr className="w-[33px] border border-[#9E8557]" />
            <p className="font-DINAMedium text-[16px] font-medium leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px] rtl:font-TajawalRegular">
              <span>{locale==="en"?"Email":"البريد الإلكتروني"}:</span> info@oia.gov.om 
            </p>
            <p className="font-DINAMedium text-[16px] font-medium leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px] rtl:font-TajawalRegular">
              <span>{locale==="en"?"Phone: ":"الهاتف:"} </span><span dir="ltr">+968 2474 5100</span>
            </p>
          </div>
          ...
          <div className="flex flex-col gap-[16px] ">
            ...
            >
              {locale==="en"?"Whistleblowing:":"الإبلاغ عن المخالفات"}
              </h5>
            <hr className="w-[33px] border border-[#9E8557]" />
            {data.callDetails.map(
              (value: any, id: number) => (
                <p
                  key={value.id}
                  className={`${setFontFamily(data?.locale || "en", "medium")} text-[16px] font-medium rtl:font-normal leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px]`}
                >
                  {value.label}:<span dir="ltr"> {value.phoneNumber}</span> 
                </p>
              ),
            )}
          </div>
          ...
          <div className="flex max-w-full flex-col gap-[16px] sm:max-w-[573px] lg:max-w-full">
            ...
            <hr className="w-[33px] border border-[#9E8557]" />
            <div>
              <p className={`${setFontFamily(data?.locale || "en", "medium")} text-[16px] font-medium rtl:font-normal leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px]`}> 
                <span>{locale === "en" ? 'Building Number:' : 'رقم المبنى:'}</span> <span>{locale === "en" ? '70' : '70'}</span> 
              </p>
              <p className={`${setFontFamily(data?.locale || "en", "medium")} text-[16px] font-medium rtl:font-normal leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px]`}> 
                <span>{locale === "en" ? 'Way:' : 'السكة:'}</span> <span>{locale === "en" ? '3430' : '3430'}</span> 
              </p>
              <p className={`${setFontFamily(data?.locale || "en", "medium")} text-[16px] font-medium rtl:font-normal leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px]`}> 
                {locale === "en" ? 'Al Khuwair, Arab League Street' : "الخوير، شارع جامعة الدول العربية"}
              </p>
              <p className={`${setFontFamily(data?.locale || "en", "medium")} text-[16px] font-medium rtl:font-normal leading-[24px] text-[#202020] sm:text-[24px] sm:leading-[36px] lg:text-[18px] lg:leading-[27px]`}> 
                <span>{locale === "en" ? 'Postal Code:' : 'الرمز البريدي:'}</span> <span>{locale === "en" ? '100, Muscat, Oman' : '100 مسقط, عُمان'}</span> 
              </p>
            </div>
          </div>
        </div>
              ...
              <Form className="h-full w-full">
                <div className="contact-form-container flex h-full w-full flex-col gap-[20px] lg:justify-between lg:gap-0">
                  ...
                  <div className="flex w-full flex-col gap-[10px]">
                    ...
                    >
                      {locale === "en"
                        ? forms.english.message
                        : forms.arabic.message}{" "}
                      *
                    </label>
      ...
      <ToastContainer
        ...
        draggable
        pauseOnHover
        className={`${locale === "en" ? "!font-TajawalMedium" : "!font-DINAMedium"}`}
      />
    </ScreenContainer>
  );
};
