...
const FutureGenerationFundAllocation:React.FC<any> = ({data}: any) => {
  
  const {title, description,locale, cards} = data;
  const respCardData = cards?.filter((card:any)=>([2,4,5,7]?.includes(card?.id)));
  return (
    <section className="fund-allocation-section bg-[#FFF9EC] lg:py-[100px] sm:py-[50px] py-[40px]">
          ...
          <div className="lg:grid grid-cols-4  gap-y-[30px] gap-x-[20px] hidden">
            {cards?.map((data:any, index:number) => {
              return data?.isImage ? (
                ...
                <div
                  key={`text-${index}`}
                  className="flex h-[300px] w-full max-w-[300px] flex-col justify-between overflow-hidden bg-[#FFFFFF] px-[20px] py-[37.5px]"
                >
...
export default FutureGenerationFundAllocation;
