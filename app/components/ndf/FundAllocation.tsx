...
const FundAllocation = ({data}: any) => {
  const {title, description, cards, locale} = data
  const respCardData = cards?.filter((card:any)=>([2,4,5,7]?.includes(card?.id)));
  return (
    <section className="fund-allocation-section bg-[url('/assets/images/ndf-imgs/bg-pattren-img.png')] bg-cover lg:py-[100px] sm:pt-[50px] sm:pb-[59px] py-[40px]">
